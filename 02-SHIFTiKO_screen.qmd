...
bar.phenotyping <- barseq %>%
  ...
  group_by(Replicate, Gene) %>%
  mutate(SD.FC = p[Timepoint == "T0sz"] / p[Timepoint == "T0"], # Calculate fold change across schizont development
         E.FC = p[Timepoint == "T0nsz"] / p[Timepoint == "T0sz"], # Calculate fold change across egress (schizonts vs unegressed schizonts)
         I.FC = p[Timepoint == "T1"] / p[Timepoint == "T0sz"], # Calculate fold change across invasion (schizonts vs rings)
         TD.FC = p[Timepoint == "T1sz"] / p[Timepoint == "T1"]) %>% # Calculate fold change across trophozoite development
  group_by(Replicate) %>%
